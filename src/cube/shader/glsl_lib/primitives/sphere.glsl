...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {{
    ...
    if (t > 0.0) {{
        ...
        // Check if we hit axes or sphere
        vec3 baseColor;
        if (iDebugAxes > 0.5) {{
            ...
            if (axesDist < 0.001) {{
                ...
                // We hit the sphere - use normal sphere coloring
                vec2 uv = getSphereUV(p);
                float checker = checkerboard(uv, 16.0);
            ...
            // Debug axes disabled - use normal sphere coloring
            vec2 uv = getSphereUV(p);
            float checker = checkerboard(uv, 16.0);
            float heightFactor = (p.y + 1.5) / 3.0;
