...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
	...
	// Gentle automatic camera motion - kept fixed on horizon
	float time = iTime * 0.3;
	vec3 ang = vec3(sin(time*3.0)*0.05, -0.05, time);
	mat3 autoRot = fromEuler(ang);
	
	// Use camera uniforms for controllable orientation
	vec3 cu = iCameraRight;
	vec3 cv = iCameraUp;
	vec3 cw = iCameraForward;
	
	// Apply gentle motion on top of user-controlled orientation
	cu = normalize(autoRot * cu);
	cv = normalize(autoRot * cv);
	cw = normalize(autoRot * cw);
	
	vec3 dir = normalize(uv.x*cu + uv.y*cv + cw);
