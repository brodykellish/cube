...
{	
	...
	float xPos = (id.x * 0.02) ;
	float normalizedX = clamp((xPos + 0.4) / 0.8, 0.0, 1.0);
	
	float pulseValue = 0.0;
	if (normalizedX > 0.95) {
		pulseValue = iParam1;
	} else {
		float timeOffset = (1.0 - normalizedX) * 2.0;
		float samplePhase = mod(iBeatPhase - timeOffset + 1.0, 1.0);
		
		if (samplePhase < 0.05) {
			pulseValue = mix(0.0, 1.0, samplePhase / 0.05);
		} else if (samplePhase < 0.15) {
			pulseValue = 1.0;
		} else {
			pulseValue = max(0.0, 1.0 - (samplePhase - 0.15) / 0.85);
		}
	}
	
	float amp = 7.0;
	float waveformY = (pulseValue - 0.5) * amp;
	float inte = abs(id.y + waveformY) - 0.8;
	acol += exp(max(v, 0.0) * -400.0) * smoothstep(0.2, 0.0, inte) * step(id.x, 21.0) * step(0.0, id.x);
	//0.018
	col += acol;
