...
{
	float zoom = mix(1.0, 15.0, clamp(iParam0, 0.0, 1.0));
	vec2 uv = (fragCoord.xy-.5*iResolution.xy) * zoom / iResolution.y;

    float r = 1.0;
    float rotSpeed = mix(0.0, 0.5, clamp(iParam1, 0.0, 1.0));
    float a = iTime * rotSpeed;
    float c = cos(a)*r;
    float s = sin(a)*r;
    
    int iterations = int(mix(4.0, 64.0, clamp(iParam2, 0.0, 1.0)));
    float offset = mix(0.0, 0.5, clamp(iParam3, 0.0, 1.0));
