...
// FIXED: Using simpler normal calculation method (no bit operations needed for GLSL 1.20)
// https://iquilezles.org/articles/normalsSDF
vec3 calcNormal( in vec3 pos, float time )
{
    // Use simple tetrahedron technique (GLSL 1.20 compatible)
...
// FIXED: Replaced ZERO trick with regular loop start (GLSL 1.20 compatibility)
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    vec3 tot = vec3(0.0);
#if AA>1
    // FIXED: Replaced ZERO with 0 for GLSL 1.20 compatibility
    for( int m=0; m<AA; m++ )
    for( int n=0; n<AA; n++ )
    {
        // pixel coordinates
