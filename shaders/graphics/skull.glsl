...
vec3 calcNormal (vec3 p){
    // GLSL 1.20 compatibility: simplified loop (removed min(iFrame,0) trick for compatibility)
    vec3 n = vec3(0.0);
    for( int i=0; i<4; i++ ){
        // GLSL 1.20 compatibility: emulate bit operations using math
        // (x>>1) = floor(x/2), (x&1) = mod(x,2)
        float fi = float(i);
        vec3 e = 0.5773*(2.0*vec3(
            mod(floor((fi+3.0)/2.0), 2.0),
