...
// iParam1: translation speed (0-1 maps to 0-0.2 UV units per second)

void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    vec2 uv = fragCoord / iResolution.xy;
    
    // Tile scale from iParam0 (zoom factor)
    // Maps 0-1 to 2.0-8.0, where smaller values = more tiles, larger values = fewer tiles
    float tileScale = mix(8.0, 3.0, clamp(iParam0, 0.0, 1.0));
    
    // Speed of translation from iParam1
    // Maps 0-1 to 0-0.2 UV units per second
    float speed = mix(0.0, 0.2, clamp(iParam1, 0.0, 1.0));
