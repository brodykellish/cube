...
void mainImage( out vec4 colOut, in vec2 fragCoord )
{
    ...
    for( int i=0; i<AA; i++ )
    {
        //setting up camera using precomputed smooth spherical camera system
        vec2 uv = (fragCoordRot - iResolution.xy * 0.5 + vec2(i,j)/(float(AA))) / iResolution.y;

        // Use precomputed camera vectors for smooth, discontinuity-free rotation
        vec3 pos = iCameraPos;
        ...
        vec3 ray = normalize(uv.x * right + uv.y * up + 1.0 * forward);
