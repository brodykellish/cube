...
vec3 RayTrace(in vec2 fragCoord )
{
    ...
    if ((t <= maxDepth) || (t == alpha))
	{
        ...
        if (distAndMat.y >= 100.0)
        {
            ...
            vec2 envUV = vec2(atan(ref.z, ref.x) / (2.0 * PI) + 0.5, acos(ref.y) / PI);
            finalColor += saturate(texture(iChannel0, envUV).xyz-0.35)*0.15*max(0.2,sunShadow);
        }
        ...
        if (windowRef != 0.0)
        {
            ...
            vec2 envUV = vec2(atan(ref.z, ref.x) / (2.0 * PI) + 0.5, acos(ref.y) / PI);
            finalColor += saturate(texture(iChannel0, envUV).xyz-0.35)*0.15*max(0.25,sunShadow)*windowRef;
        }
        finalColor *= 0.9;
