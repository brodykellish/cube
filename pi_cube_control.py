...
REQUIRES: Raspberry Pi 5 with piomatter C extension installed.
For development/testing on macOS/Windows, use cube_control.py instead.
"""
...
class PiCubeController:
    ...
    def __init__(
        ...
        **kwargs
        ...
        # Use piomatter backend only (required for Raspberry Pi)
        try:
            import piomatter
            if not piomatter._PIOMATTER_AVAILABLE:
                raise RuntimeError(
                    'piomatter C extension is not available. '
                    'This script requires a Raspberry Pi 5 with the piomatter extension built correctly. '
                    'Make sure you are running on a Raspberry Pi 5 and the extension is installed.'
                )
            from cube.display.piomatter_backend import PiomatterBackend
            self.backend = PiomatterBackend(width, height, **kwargs)
            print("[PI] Using piomatter backend")
        except (ImportError, RuntimeError) as e:
            print(f"\n{'='*60}")
            print("ERROR: piomatter backend is required")
            print(f"{'='*60}")
            print(f"{e}")
            print("\nThis script is designed for Raspberry Pi 5 deployment only.")
            print("For development/testing, use cube_control.py instead.")
            print(f"{'='*60}\n")
            raise
